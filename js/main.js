...
//funcion autoejecutable de menu principal
(() => {
    ...
    const menuPrincipal=async()=>{
        let opcionPrincipal = Number(prompt(`
        ...
        if (opcionPrincipal === 1) {
            ...
            alert('Papi solo se aceptan números, nada de espacios en blanco ni letras ☠');
            menuPrincipal();
        }
    };
// -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    const menuPosts=async()=>{
        let opcionPosts = Number(prompt(`
            MENU POSTS 📠
            1. Obtener todos los datos🗺
            2. Obtener un dato exacto🗃
            3. Publicar un dato nuevo📮
            4. Actualizar un dato completo🛠
            5. Actualizar valor de un dato exacto🔧
            0. Salir 🚨`, "Ingresa el numero"));

        if (opcionPosts === 1) {
            alert('Para la info completa revisa la consola (ctrl+shift+c)...continuar');
            const res = await getAllPosts(); // Asumiendo que esta función ya está definida
            console.table(res);
            const listdata = res.map(post => `Id: ${post.id}, Title: ${post.title}`).join('\n');
            alert(listdata);
            menuPosts(); // Volver al menú después de mostrar la información


        } else if (opcionPosts === 2) {
            // Código para obtener un dato exacto
            let input=Number(prompt('Introduce el id que deseas observar:'));
           
            //condicion para que sea numero
            if (isNaN(input)){alert('Error, dato no valido');menuPosts();}

            //impresion de los datos
            const res=await getOnePosts(input); //imprimir en consola, tabla, completo
            console.table(res);

            const listdata = JSON.stringify(res); //imprimir por alerta, visual
            alert(listdata);
            
            menuPosts(); //retorno despues de haber encontrado el dato


        } else if (opcionPosts === 3) {
            // Código para publicar un dato nuevo
            menuPosts();
        } else if (opcionPosts === 4) {
            // Código para actualizar un dato completo
            menuPosts();
        } else if (opcionPosts === 5) {
            // Código para actualizar valor de un dato exacto
            menuPosts();
        } else if (opcionPosts === 0) {
            menuPrincipal();
        } else {
            alert('Papi solo se aceptan números ☠');
            menuPosts();
        }
    };
    ...
    const menuComments=async()=>{
        let opcionComments = Number(prompt(`
        MENU COMMENTS 📥
        1. Obtener todos los datos🗺
        2. Obtener un dato exacto🗃
        3. Publicar un dato nuevo📮
        ...
        if (opcionComments === 1) {
            alert('Para la info completa revisa la consola (ctrl+shift+c)...continuar');
            const res = await getAllComments(); // Asumiendo que esta función ya está definida
            console.table(res);
            const listdata = res.map(val=>`Id:${val.id},Name:${val.name},Body:${val.body}`).join('\n');
            alert(listdata);
            menuComments(); // Volver al menú después de mostrar la información


        } else if (opcionComments === 2) {
            // Código para obtener un dato exacto
            let input=Number(prompt('Introduce el id que deseas observar:'));
           
            //condicion para que sea numero
            if (isNaN(input)){alert('Error, dato no valido');menuComments();}

            //impresion de los datos
            const res=await getOneComment(input); //imprimir en consola, tabla, completo
            console.table(res);

            const listdata = JSON.stringify(res); //imprimir por alerta, visual
            alert(listdata);
            
            menuComments(); //retorno despues de haber encontrado el dato


        } else if (opcionComments === 3) {
            // Código para publicar un dato nuevo
            menuComments();
        } else if (opcionComments === 4) {
            // Código para actualizar un dato completo
            menuComments();
        } else if (opcionComments === 5) {
            // Código para actualizar valor de un dato exacto
            menuComments();
        } else if (opcionComments === 0) {
            menuPrincipal();
        } else {
            alert('Papi solo se aceptan números ☠');
            menuComments();
 ...
 // -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    const menuAlbums=async()=>{
        let opcionAlbums = Number(prompt(`
        MENU ALBUMS 💽
        1. Obtener todos los datos🗺
        2. Obtener un dato exacto🗃
        3. Publicar un dato nuevo📮
        ...
        if (opcionAlbums === 1) {
            alert('Para la info completa revisa la consola (ctrl+shift+c)...continuar');
            const res = await getAllAlbums(); // Asumiendo que esta función ya está definida
            console.table(res);
            const listdata = res.map(val => `Id:${val.id}, Title:${val.title}`).join('\n');
            alert(listdata);
            menuAlbums(); // Volver al menú después de mostrar la información


        } else if (opcionAlbums===2) {
            // Código para obtener un dato exacto
            let input=Number(prompt('Introduce el id que deseas observar:'));
           
            //condicion para que sea numero
            if (isNaN(input)){alert('Error, dato no valido');menuAlbums();}

            //impresion de los datos
            const res=await getOneAlbum(input); //imprimir en consola, tabla, completo
            console.table(res);

            const listdata = JSON.stringify(res); //imprimir por alerta, visual
            alert(listdata);
            
            menuAlbums(); //retorno despues de haber encontrado el dato



        } else if (opcionAlbums===3) {
            // Código para publicar un dato nuevo
            menuAlbums();
        } else if (opcionAlbums===4) {
            // Código para actualizar un dato completo
            menuAlbums();
        } else if (opcionAlbums===5) {
            // Código para actualizar valor de un dato exacto
            menuAlbums();
        } else if (opcionAlbums===0) {
            menuPrincipal();
        } else {
            alert('Papi solo se aceptan números ☠');
            menuAlbums();
...
// -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    const menuPhotos=async()=>{
        let opcionPhotos= Number(prompt(`
        MENU PHOTOS 📷
        1. Obtener todos los datos🗺
        2. Obtener un dato exacto🗃
        3. Publicar un dato nuevo📮
        ...
        if (opcionPhotos === 1) {
            alert('Para la info completa revisa la consola (ctrl+shift+c)...continuar');
            const res = await getAllPhotos(); // Asumiendo que esta función ya está definida
            console.table(res);
            const listdata = res.map(val => `Id:${val.id},Title:${val.title}`).join('\n');
            alert(listdata);
            menuPosts(); // Volver al menú después de mostrar la información
        ...
        } else if (opcionPhotos===2) {
            // Código para obtener un dato exacto
            let input=Number(prompt('Introduce el id que deseas observar:'));
           
            //condicion para que sea numero
            if (isNaN(input)){alert('Error, dato no valido');menuPhotos();}

            //impresion de los datos
            const res=await getOnePhoto(input); //imprimir en consola, tabla, completo
            console.table(res);

            const listdata = JSON.stringify(res); //imprimir por alerta, visual
            alert(listdata);
            
            menuPhotos(); //retorno despues de haber encontrado el dato


        } else if (opcionPhotos===3) {
            // Código para publicar un dato nuevo
            menuPhotos();
        } else if (opcionPhotos===4) {
            // Código para actualizar un dato completo
            menuPhotos();
        } else if (opcionPhotos===5) {
            // Código para actualizar valor de un dato exacto
            menuPhotos();
        } else if (opcionPhotos===0) {
            menuPrincipal();
        } else {
            alert('Papi solo se aceptan números ☠');
            menuPhotos();
...
// -------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    const menuUsers=async()=>{
        let opcionUsers = Number(prompt(`
        MENU USERS 👨‍⚕️👩‍⚕️
        1. Obtener todos los datos🗺
        2. Obtener un dato exacto🗃
        3. Publicar un dato nuevo📮
        ...
        if (opcionUsers === 1) {
            alert('Para la info completa revisa la consola (ctrl+shift+c)...continuar');
            const res = await getAllUsers(); // Asumiendo que esta función ya está definida
            console.table(res);
            const listdata = res.map(val=>`Id:${val.id},Title:${val.title}`).join('\n');
            alert(listdata);
            menuUsers(); // Volver al menú después de mostrar la información


        } else if (opcionUsers === 2) {
            // Código para obtener un dato exacto
            let input=Number(prompt('Introduce el id que deseas observar:'));
           
            //condicion para que sea numero
            if (isNaN(input)){alert('Error, dato no valido');menuUsers();}
            ...
            menuUsers(); //retorno despues de haber encontrado el dato


        } else if (opcionUsers===3) {
            // Código para publicar un dato nuevo
            menuUsers();
        } else if (opcionUsers===4) {
            // Código para actualizar un dato completo
            menuUsers();
        } else if (opcionUsers===5) {
            // Código para actualizar valor de un dato exacto
            menuUsers();
        } else if (opcionUsers===0) {
            menuPrincipal();
        } else {
            alert('Papi solo se aceptan números ☠');
            menuUsers();
    ...
    menuPrincipal(); //Activar todo el codigo de menu principal
})();
